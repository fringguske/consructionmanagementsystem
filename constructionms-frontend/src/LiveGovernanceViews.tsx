...
export function OpeningPositionsView({ currentUser }: { currentUser: CurrentUser }) {
  ...
  useEffect(() => {
    const controller = new AbortController()
    Promise.all([openingPositionsApi.list(undefined, controller.signal), currentUser.role === 'Storekeeper' ? materialsApi.list({ page: 1, pageSize: 100 }, controller.signal) : Promise.resolve(null)])
      .then(([response, materialResponse]) => { setPositions(response); setMaterials(materialResponse?.items ?? []); setError(null) })
      .catch(cause => { if (!(cause instanceof DOMException && cause.name === 'AbortError')) setError(messageOf(cause)) })
      .finally(() => { if (!controller.signal.aborted) setLoading(false) })
    return () => controller.abort()
  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    if (saved) setForm({ projectId: '', materialId: '', quantity: '', unitCost: '', accountName: '', amount: '', asOfDate: today, evidenceReference: '', notes: '' })
  }
...
export function CustodyCloseoutView({ currentUser }: { currentUser: CurrentUser }) {
  ...
  useEffect(() => {
    const controller = new AbortController()
    Promise.all([inventoryApi.issues(controller.signal), custodyControlsApi.returns(undefined, controller.signal), custodyControlsApi.closeouts(undefined, controller.signal)])
      .then(([issueResponse, returnResponse, closeoutResponse]) => { setIssues(issueResponse.items); setReturns(returnResponse); setCloseouts(closeoutResponse); setError(null) })
      .catch(cause => { if (!(cause instanceof DOMException && cause.name === 'AbortError')) setError(messageOf(cause)) })
      .finally(() => { if (!controller.signal.aborted) setLoading(false) })
    return () => controller.abort()
  }, [refresh])
...
export function PeriodClosingView({ currentUser }: { currentUser: CurrentUser }) {
  ...
  useEffect(() => {
    const controller = new AbortController()
    Promise.all([accountingPeriodsApi.list(undefined, controller.signal), accountingPeriodsApi.corrections(undefined, controller.signal), currentUser.role === 'Storekeeper' ? materialsApi.list({ page: 1, pageSize: 100 }, controller.signal) : Promise.resolve(null)])
      .then(([periodResponse, correctionResponse, materialResponse]) => { setPeriods(periodResponse); setCorrections(correctionResponse); setMaterials(materialResponse?.items ?? []); setError(null) })
      .catch(cause => { if (!(cause instanceof DOMException && cause.name === 'AbortError')) setError(messageOf(cause)) })
      .finally(() => { if (!controller.signal.aborted) setLoading(false) })
    return () => controller.abort()
  }, [currentUser.role, refresh])
  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    if (saved) setForm({ projectId: '', name: '', startDate: monthStart, endDate: today })
  }
  ...
  return <div className="lav-view governance-view ceo-readable">
    <header className="lav-page-head"><div><span className="lav-kicker">Operational cut-off</span><h1>Period closing & corrections</h1></div><span className="lav-count-chip">{periods.filter(item => item.status !== 'Closed').length} open</span></header>
    {error && <Notice tone="error">{error}</Notice>}{notice && <Notice tone="success">{notice}</Notice>}
