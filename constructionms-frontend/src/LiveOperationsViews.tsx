...
export function LiveInventoryView({ currentUser }: { currentUser: CurrentUser }) {
  ...
  return <div className="lav-view ops-view">
    <header className="lav-page-head"><div><span className="lav-kicker">MATERIAL CUSTODY</span><h1>{role === 'Foreman' ? 'Materials issued to me' : 'Stock and movement'}</h1><p>Every quantity is tied to its catalog unit and source document.</p></div></header>
    {error && <Notice tone="error">{error}</Notice>}{notice && <Notice tone="success">{notice}</Notice>}
    {role === 'Storekeeper' && <StorekeeperActions currentUser={currentUser} orders={orders} receipts={receipts} requisitions={requisitions} balances={balances} materials={materials} issues={issues} transfers={transfers} counts={counts} onChanged={changed}/>}
    ...
    {role !== 'Foreman' && <section className="lav-panel ops-panel"><header className="lav-panel-head"><div><span className="lav-kicker">MOVEMENT LEDGER</span><h2>Latest store movements</h2></div></header>
      {ledger.length ? <div className="ops-table"><div className="ops-row head"><span>Material</span><span>Movement</span><span>Quantity</span><span>Balance</span><span>Proof</span></div>{ledger.slice(0, 12).map(item => <div className="ops-row" key={item.id}><span><b>{item.materialName}</b><small>{item.projectName}</small></span><span>{item.movementType}</span><span className={item.quantityDelta < 0 ? 'negative' : 'positive'}>{item.quantityDelta > 0 ? '+' : ''}{item.quantityDelta} {item.unit}</span><span>{item.balanceAfter} {item.unit}</span><span><b>{item.referenceNumber}</b><small>{item.actorName} · {when(item.occurredAt)}</small></span></div>)}</div> : <Empty>No receipts, issues, transfers or count adjustments yet.</Empty>}
    </section>}
...
function StorekeeperActions({ currentUser, orders, receipts, requisitions, balances, materials, issues, transfers, counts, onChanged }: { currentUser: CurrentUser; orders: PurchaseOrder[]; receipts: GoodsReceipt[]; requisitions: Requisition[]; balances: StockBalance[]; materials: Material[]; issues: MaterialIssue[]; transfers: StockTransfer[]; counts: StockCount[]; onChanged: (text: string) => void }) {
  ...
  const availableReqs = requisitions.filter(requisition => !issues.some(issue => issue.requisitionId === requisition.id))
  ...
  return <section className="ops-action-grid">
    ...
    <form className="lav-panel ops-form" onSubmit={event => { event.preventDefault(); void submit(() => inventoryApi.issue({ requisitionId: Number(issuing.requisitionId), quantity: Number(issuing.quantity), notes: issuing.notes || null }), 'Material issue voucher saved; the Foreman must confirm receipt.').then(saved => { if (saved) setIssuing({ requisitionId: '', quantity: '', notes: '' }) }) }}>
      <h2>Issue approved material</h2><p>Stock leaves only against a Supervisor-approved request.</p>
      <label><span>Approved request</span><select required value={issuing.requisitionId} onChange={e => { const req = requisitions.find(item => item.id === Number(e.target.value)); setIssuing({ ...issuing, requisitionId: e.target.value, quantity: req ? String(req.quantity) : '' }) }}><option value="">Choose request</option>{availableReqs.map(req => <option value={req.id} key={req.id}>MR-{req.id} · {req.materialName} · {req.projectName}</option>)}</select></label>
    ...
    <form className="lav-panel ops-form" onSubmit={event => { event.preventDefault(); void submit(() => inventoryApi.createCount({ projectId: Number(count.projectId), materialId: Number(count.materialId), countedQuantity: Number(count.quantity), notes: count.notes }), 'Physical count submitted for independent Supervisor review.').then(saved => { if (saved) setCount({ projectId: '', materialId: '', quantity: '', notes: '' }) }) }}>
      <h2>Submit a stock count</h2><p>The Supervisor reviews any difference before stock changes.</p>
...
function InvoiceCard({ invoice, role, run }: { invoice: SupplierInvoice; role: CurrentUser['role']; run: (action: () => Promise<unknown>, text: string) => Promise<boolean> }) {
  const action = () => {
    ...
    if (role === 'CEO' && invoice.status === 'AwaitingCeoApproval') return <div className="ops-buttons"><button className="lav-button primary" onClick={() => void run(() => financeApi.ceoDecision(invoice.id, true, 'High-value exception approved after reviewing the complete evidence chain'), 'CEO exception approved; Finance may authorize.')}>Approve exception</button><button className="lav-button secondary" onClick={() => void run(() => financeApi.ceoDecision(invoice.id, false, 'High-value exception rejected by CEO'), 'CEO exception rejected.')}>Reject</button></div>
    return null
  }
  return <article><header><div><span>{invoice.projectName}</span><h3>{invoice.invoiceNumber}</h3><small>{invoice.purchaseOrderNumber} · {invoice.supplierName}</small></div><b className={`ops-status ${invoice.status.toLowerCase()}`}>{invoice.status.replaceAll(/([A-Z])/g, ' $1').trim()}</b></header><strong>{money(invoice.amount)}</strong><p>{invoice.quantity} {invoice.materialUnit} of {invoice.materialName}</p>{invoice.reviewedAt && <div className="ops-match"><span className={invoice.quantityMatches ? 'pass' : 'fail'}>Quantity {invoice.quantityMatches ? 'matches' : 'differs'}</span><span className={invoice.priceMatches ? 'pass' : 'fail'}>Price {invoice.priceMatches ? 'matches' : 'differs'}</span><span className={invoice.amountMatches ? 'pass' : 'fail'}>Total {invoice.amountMatches ? 'matches' : 'differs'}</span></div>}<footer><small>Captured by {invoice.capturedByName} · {when(invoice.capturedAt)}</small>{action()}</footer></article>
}
...
function PettyCashCard({ item, currentUser, run }: { item: PettyCashRequest; currentUser: CurrentUser; run: (action: () => Promise<unknown>, text: string) => Promise<boolean> }) {
  ...
  return <article className="petty-cash-card">
    <header><div><span>{item.requestNumber} · {item.projectName}</span><h3>{item.purpose}</h3><small>{item.costCode} · requested by {item.requestedByName}</small></div><b className={`ops-status ${item.status.toLowerCase()}`}>{item.status.replaceAll(/([A-Z])/g, ' $1').trim()}</b></header>
