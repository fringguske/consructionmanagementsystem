...
function StorekeeperActions({ currentUser, projectSummaries, orders, receipts, requisitions, balances, materials, issues, transfers, counts, onChanged }: { currentUser: CurrentUser; projectSummaries: ProjectSummary[]; orders: PurchaseOrder[]; receipts: GoodsReceipt[]; requisitions: Requisition[]; balances: StockBalance[]; materials: Material[]; issues: MaterialIssue[]; transfers: StockTransfer[]; counts: StockCount[]; onChanged: (text: string) => void }) {
  ...
  return <section className="ops-action-grid">
    <form className="lav-panel ops-form ops-replenishment" onSubmit={event => { event.preventDefault(); void submit(() => requisitionsApi.createStockReplenishment({ projectId: Number(replenishment.projectId), materialId: Number(replenishment.materialId), costCodeId: Number(replenishment.costCodeId), quantity: Number(replenishment.quantity), neededByDate: replenishment.neededByDate, reason: replenishment.reason, notes: replenishment.notes.trim() || null }), `Store replenishment request sent. Wait for the Supervisor assigned to ${replenishmentProject?.project.name ?? 'this project'} to approve it before Procurement sources suppliers.`).then(saved => { if (saved) setReplenishment({ projectId: '', costCodeId: '', materialId: '', quantity: '', neededByDate: '', reason: '', notes: '' }) }) }}>
      <h2>Replenish the store</h2><p>Order reserve stock here. This is separate from the Foreman’s daily site-use request.</p>{error && <Notice tone="error">{error}</Notice>}
